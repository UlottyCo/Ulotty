...
export function UsuariosTablaClient({ filterRole, filterVerified }: UsuariosTablaClientProps) {
  ...
  const handleSuspend = async (id: string, currentStatus: boolean) => {
    try {
      ...
      showToast(currentStatus ? "Usuario suspendido" : "Usuario reactivado", "success");
    } catch (error) {
            ...
            {users.map((user) => (
              <tr key={user.id} className="border-b border-border hover:bg-background transition">
                <td className="py-3 px-4">
                  ...
                  <div className="text-xs text-muted">{user.id?.substring(0, 8)}...</div>
                </td>
                ...
                <td className="py-3 px-4">
                  <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                    user.active ? "bg-blue-100 text-blue-700" : "bg-red-100 text-red-700"
                  }`}>
                ...
                <td className="py-3 px-4">
                  <div className="flex gap-1">
                    <button
                      onClick={() => router.push(`/panel-admin/usuarios/${user.id}`)}
                    ...
                    <button
                      onClick={() => handleSuspend(user.id, user.active)}
